...
# Connect this script to your Django project/database
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "RootQR.settings")
django.setup()

from QRCode_Main.models import Student
# ── Token generation ──────────────────────────────────────────────────────────
...
# ── Generate QR Codes and save tokens to database ─────────────────────────────

existing_qr_codes = list(QR_DIR.glob("*.png"))

if existing_qr_codes:
    print(f"Found {len(existing_qr_codes)} existing QR codes.")
    print("Adding their tokens to the database...")

    for qr_path in existing_qr_codes:
        token = qr_path.stem
    ...
    print("Existing QR code tokens saved to database.")

else:
    print("No QR codes found. Generating 200 new QR codes...")

    tokens = generate_tokens(amount=200, length=12)
    ...
    for token in tokens:
        ...
        qr_img.save(QR_DIR / f"{token}.png")

    print("200 QR codes generated and saved to database.")
